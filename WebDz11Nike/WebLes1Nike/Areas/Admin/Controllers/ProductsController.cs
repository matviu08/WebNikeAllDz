...
using WebLes1Nike.Areas.Admin.Models.Product;
...
namespace WebLes1Nike.Areas.Admin.Controllers
{
    ...
    public class ProductsController(NikeDbContext nikeDbContext, IImageService imageService) : Controller
    {
        ...
        {
            List<ProductItemVM> model = nikeDbContext.Products
        ...
        {
            ProductDetailVM? model = nikeDbContext.Products
        ...
        public IActionResult ProdEdit(int id)
        {
            ...
            var model = new ProductEditVM()
            {
                ...
                ExistingImages = product.ProductImages
                    .OrderBy(x => x.Order)
                    .Select(x => new ExistingProductImageVM()
                    {
                        Id = x.Id,
                        Name = x.Name,
                        Order = x.Order
        ...
        [HttpPost]
        public async Task<IActionResult> ProdEdit(ProductEditVM model)
        {
            if (!ModelState.IsValid)
            {
                ViewBag.Categories = nikeDbContext.Categories.Where(x => !x.isDeleted).Select(x => x.Name).ToList();
